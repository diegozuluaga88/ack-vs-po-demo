...
| DE1.18 | (a) OCR · removido botón grande "Compare linked documents" + prop `onCompareLinked` + import `ArrowLeftRight` + `<ComparisonLauncher>` mounted + state `compareDoc` \| (b) Comparisons · cards rediseñadas al layout OCR-style vía nuevo `wrappers/ComparisonDocCard.tsx` (doc icon + vendor + chip tipo + avatar · Filename + Line Items · footer con Reviewed/Pending + Preview + Compare + Delete + Send) \| (c) `ComparisonDoc` shape ampliado (`type`, `name`, `subCode`) + mocks con 5 nuevos PO entries \| (d) Tabs sincronizados con prod: **Purchase Orders / Acknowledgements** (2 tabs sin All · particiona por tipo, no por status) | `src/OCRTracking.tsx` · `src/components/ocr/OcrDocCard.tsx` · `src/Comparisons.tsx` · `src/components/comparison/wrappers/ComparisonDocCard.tsx` (nuevo) |
